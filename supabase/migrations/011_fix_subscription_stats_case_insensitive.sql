-- ============================================================================
-- 修复订阅统计函数 - 统一使用不区分大小写的 Free 计划过滤
-- ============================================================================

-- 更新 get_admin_dashboard_stats 函数，使用 LOWER() 进行大小写不敏感比较
      ...
      SELECT jsonb_build_object(
        ...
        'by_plan', (
          ...
          FROM (
            ...
            WHERE (p_source = 'all' OR source = p_source)
              AND LOWER(plan) != 'free'  -- 修复：使用 LOWER() 进行不区分大小写的比较
            GROUP BY plan
          ) sub
        )
      )
...
-- 说明
-- ============================================================================
-- 本次修复解决了订阅统计中的大小写不一致问题：
-- 1. Supabase 原先使用 plan != 'Free' (严格区分大小写)
-- 2. CloudBase 使用 plan.toLowerCase() !== "free" (不区分大小写)
-- 3. 现在统一使用 LOWER(plan) != 'free' 确保两个版本的统计结果一致
-- ============================================================================
