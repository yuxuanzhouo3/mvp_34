...
{
    // Check if privacy dialog should be shown
    if (![PrivacyDialogHelper hasAgreed]) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            UIViewController *rvc = self.window.rootViewController;
            [PrivacyDialogHelper showPrivacyDialogFrom:rvc onAgree:^{
                // User agreed, continue with app setup
                [self continueAppSetup:application];
            }];
        });
        return;
    }
...
- (void)continueAppSetup:(UIApplication *)application
{
    GoNativeAppConfig *appConfig = [GoNativeAppConfig sharedAppConfig];
