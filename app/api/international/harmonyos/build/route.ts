...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!bundleNameRegex.test(bundleName)) {
      return NextResponse.json(
        { error: "Invalid bundle name", message: "Bundle name should be in format: com.example_harmony.app" },
        { status: 400 }
      );
    }
