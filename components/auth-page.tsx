    ...
    try {
      ...
      if (mode === "signup") {
        ...
        if (isDomesticVersion) {
          ...
          // 邀请绑定：如果用户是通过 /r/[shareCode] 进来的，这里会携带 mk_ref cookie
          // 注册成功后把邀请关系绑定到新用户上（不影响注册主流程）
          const invitedUserId = String(data?.user?.id || "").trim();
          if (invitedUserId) {
            try {
              const bindRes = await fetch("/api/invite/bind", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ invitedUserId }),
              });
              const bindData = await bindRes.json().catch(() => ({}));
              console.log("[AuthPage] Invite bind result:", { ok: bindRes.ok, ...bindData });
            } catch (bindErr) {
              console.warn("[AuthPage] Invite bind failed (ignored):", bindErr);
            }
          }
          ...
          // 注册成功后尝试绑定邀请关系并发放注册奖励
          try {
            if (data.user?.id) {
              fetch("/api/invite/bind", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ invitedUserId: data.user.id }),
                credentials: "include",
              }).catch(() => {});
            }
          } catch {
        ...
        if (isDomesticVersion) {
          ...
          if (!res.ok) {
            ...
            toast.error(loginMessage || data.error || (isZhText ? "登录失败" : "Login failed"));
            return;
          }
          ...
          if (error) {
            ...
            toast.error(loginMessage || error.message);
            return;
          }
          ...
          if (!data.user?.email_confirmed_at) {
            await supabase.auth.signOut();
            toast.error(isZhText ? "请先验证您的邮箱" : "Please verify your email first");
            return;
          }

          // 尝试绑定邀请关系并发放积分（如果当前会话带有邀请 cookie）
          try {
            if (data.user?.id) {
              fetch("/api/invite/bind", {
                method: "POST",
  ...
  const handleWechatSignIn = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.qrcodeUrl) {
        // 处理特定的错误代码，显示更友好的提示
        if (data.code === "LOCALHOST_NOT_SUPPORTED") {
          const errorMsg = isZhText
            ? "微信登录在本地开发环境中不可用。请使用内网穿透工具（如 ngrok）获取公网域名，或在生产环境中使用。"
            : "WeChat login is not available in localhost. Please use a tunneling tool (e.g., ngrok) or use in production.";
          toast.error(errorMsg, { duration: 6000 });
          console.warn("[AuthPage] WeChat login blocked in localhost:", data.message);
        } else if (data.code === "INVALID_REDIRECT_URI") {
          const errorMsg = isZhText
            ? "回调地址配置错误。请确保 NEXT_PUBLIC_APP_URL 不包含非标准端口号。"
            : "Invalid redirect URI. Please ensure NEXT_PUBLIC_APP_URL does not contain non-standard ports.";
          toast.error(errorMsg, { duration: 6000 });
          console.warn("[AuthPage] Invalid redirect URI:", data.message);
        } else {
          throw new Error(data.error || data.message || (isZhText ? "微信登录失败" : "WeChat login failed"));
        }
        setIsLoading(false);
        return;
      }
            ...
            <>
              ...
              <div className="relative mb-4">
                ...
                <div className="relative flex justify-center text-xs uppercase">
                  <span className="bg-card px-2 text-muted-foreground" suppressHydrationWarning>
