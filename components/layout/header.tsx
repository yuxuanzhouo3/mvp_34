...
export function Header() {
  ...
  useEffect(() => {
    const handleQuotaRefresh = () => {
      console.log("[Header] quota:refresh event received");
      fetchUserPlan();
    };
  ...
  const goLogin = () => {
    ...
    // 使用完整跳转，行为与在地址栏输入 /auth/login 完全一致
        ...
        <div className="container mx-auto flex h-14 sm:h-16 items-center justify-between px-3 sm:px-4 md:px-6">
          <div className="flex items-center gap-4 sm:gap-6 md:gap-8">
            <Link href="/" className="flex items-center gap-2 sm:gap-2.5 group">
              ...
              <span
                className="text-lg sm:text-xl font-bold bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-600 bg-clip-text text-transparent notranslate"
                translate="no"
            ...
            <nav className="hidden md:flex items-center gap-1">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="px-3 lg:px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground rounded-lg hover:bg-accent transition-all"
                >
                  {link.label}
                </Link>
              ))}
            ...
            <div className="hidden md:flex items-center gap-1.5 lg:gap-2">
              {user && (
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-8 lg:h-9 gap-1.5 lg:gap-2 text-amber-600 dark:text-amber-400 hover:text-amber-500 hover:bg-amber-500/10"
              ...
              {loading ? (
                <div className="h-8 lg:h-9 w-8 lg:w-9 flex items-center justify-center">
                ...
                <Button
                  size="sm"
                  onClick={goLogin}
                  className="h-8 lg:h-9 px-3 lg:px-4 gap-1.5 lg:gap-2 bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-600 hover:from-cyan-400 hover:via-blue-400 hover:to-purple-500 text-white border-0 shadow-lg shadow-cyan-500/20 hover:shadow-xl hover:shadow-cyan-500/30 transition-all"
                >
                  <LogIn className="h-3.5 w-3.5 lg:h-4 lg:w-4" />
                  <span className="text-xs lg:text-sm font-medium">{t("nav.login")}</span>
                </Button>
              )}
            </div>
            <Button
            ...
            <nav className="container mx-auto px-3 sm:px-4 py-3 sm:py-4 flex flex-col gap-1">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="py-2.5 sm:py-3 px-3 sm:px-4 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-accent rounded-lg sm:rounded-xl transition-all active:scale-[0.98]"
              ...
              <div className="flex flex-col gap-2 mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-border/50">
                {user && (
                  <Button
                    variant="outline"
                    size="sm"
                    className="h-10 sm:h-11 gap-2 border-amber-500/30 text-amber-600 dark:text-amber-400 hover:bg-amber-500/10 rounded-lg sm:rounded-xl"
                    onClick={() => {
                ...
                {loading ? (
                  ...
                  <Button
                    size="sm"
                    className="w-full h-10 sm:h-11 gap-2 bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-600 text-white rounded-lg sm:rounded-xl"
                    onClick={() => {
                      setMobileMenuOpen(false);
                      goLogin();
                    }}
