...
import { Package, Image, AlertTriangle, Apple, AlertCircle } from "lucide-react";
import { useState, useRef } from "react";

interface MacOSConfigProps {
  name: string;
  onNameChange: (value: string) => void;
  onIconChange: (file: File | null) => void;
}

export function MacOSConfig({
  name,
  onNameChange,
  onIconChange,
}: MacOSConfigProps) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-gradient-to-br from-gray-400/20 to-gray-500/20 flex items-center justify-center">
          <Apple className="h-4 w-4 sm:h-5 sm:w-5 text-gray-600 dark:text-gray-400" />
        </div>
        <div>
          <h3 className="text-base sm:text-lg font-semibold">
            {currentLanguage === "zh" ? "macOS 应用配置" : "macOS App Configuration"}
          </h3>
          <p className="text-sm text-muted-foreground">
            {currentLanguage === "zh"
              ? "将网站打包为 macOS 桌面应用"
              : "Package your website as a macOS desktop app"}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          {currentLanguage === "zh"
            ? "应用名称将显示在窗口标题和 .app 文件名中"
            : "App name will be shown in window title and .app filename"}
        </p>
      </div>
      ...
      <div className="space-y-3">
        ...
        {!iconUploadEnabled ? (
          ...
          <div
            ...
            className="border-2 border-dashed border-border/50 rounded-xl p-6 text-center cursor-pointer hover:border-gray-500/50 hover:bg-gray-500/5 transition-all duration-200"
          >
            {iconPreview ? (
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-xs text-muted-foreground/70">
                  {currentLanguage === "zh"
                    ? `推荐尺寸: 512x512 像素或更大 (最大 ${maxImageUploadMB}MB)`
                    : `Recommended: 512x512 pixels or larger (max ${maxImageUploadMB}MB)`}
                </p>
              </div>
            )}
          </div>
      ...
      <div className="rounded-xl bg-gray-500/10 border border-gray-500/20 p-4">
        <div className="flex items-start gap-3">
          <Apple className="h-4 w-4 sm:h-5 sm:w-5 text-gray-600 dark:text-gray-400 mt-0.5 flex-shrink-0" />
          <div>
            ...
            <ul className="text-sm text-muted-foreground space-y-1">
              <li>
                {currentLanguage === "zh"
                  ? "• 下载 ZIP 文件后解压，双击 .app 文件即可运行"
                  : "• Extract the ZIP file, double-click the .app to run"}
              </li>
              <li>
                {currentLanguage === "zh"
                  ? "• 可将 .app 拖入「应用程序」文件夹安装"
                  : "• Drag .app to Applications folder to install"}
              </li>
              <li>
                {currentLanguage === "zh"
                  ? "• 基于 Tauri 构建，体积小巧（约 5-8MB）"
                  : "• Built with Tauri, lightweight (~5-8MB)"}
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <ul className="text-sm text-muted-foreground space-y-1">
              <li>
                {currentLanguage === "zh"
                  ? "• 应用程序没有 Apple 签名，macOS 可能显示安全警告"
                  : "• App is unsigned, macOS may show security warnings"}
              </li>
              <li>
                {currentLanguage === "zh"
                  ? "• 首次运行时右键点击 .app → 选择「打开」"
